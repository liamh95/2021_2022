...
\subsection{What is a Digital Signature?}
Your phone probably downloads software updates every now and then.
How does your phone know that the updates are actually coming from the developer and not some hacker?
The answer is that the developer can ``sign'' the software update and your phone has a way of verifying this signature.

This scenario differs slightly from the one we've seen with RSA and ElGamal.
With RSA, Bob wants to be able to securely read messages that anybody sends him.
In this digital signature scenario, Bob wants to broadcast messages (or software updates in the previous example) and anybody should be able to verify that these messages are indeed coming from Bob.

Even though these scenarios sound like opposites in a way, we can use ideas from public key cryptography to approach both of them.
Here's such an approach to digital signatures.

\begin{enumerate}
    \item The author has a private \emph{signing key}, $k^{s}$.
    \item The author publishes a public \emph{verification key}, $k^v$.
    \item A \emph{signing algorithm} takes a document $D$ from the author along with their signing key $k^s$ and outputs a signature $D^s$ for the document.
    \item A \emph{verification algorithm} takes a document $D$, a signature $D^s$, and a verification key $k^v$.
    The algorithm returns TRUE if $D^s$ is a signature for $D$ associated with the signing key $k^s$.
    Otherwise, it returns FALSE.
\end{enumerate}

It's important to note that the verification algorithm does \emph{not} know what the signing key is.
What properties should signing and verifying have?
Suppose an attacker knows the verification key and some signatures $D_1^s, \ldots, D_n^s$ for some documents $D_1, \ldots, D_s$.
Then the following are nice properties to have.

\begin{itemize}
    \item (Resistance to total breaks) An attacker should not be able to determine the signing key.

    \item (Resistance to universal forgery) An attacker shouldn't be able to sign arbitrary documents.

    \item (Resistance to selective forgery) An attacker shouldn't be able to sign a document of their choice.

    \item (Resistance to existential forgery) An attacker shouldn't be able to sign some document they don't already know the signature for.
\end{itemize}

\begin{remark}
    In practice, it would be far too slow to input an entire document (think of a large file like an HD movie) and some signing algorithms only accept documents of a fixed small size.
    Instead, we usually sign a \emph{hash} of the document.
    A hash function takes in documents of arbitrary size and spits out, say, a bit string of a fixed size,
    \[
        H: \{\text{arbitrary documents}\}\to \{0,1\}^k
    \]
    and is hard to invert or find collisions for.
    Instead of signing $D$, we sign $H(D)$.
\end{remark}
...
\subsection{RSA Digital Signatures}
It turns out that we can use RSA for signatures too (both uses of RSA were outlined in the original 1977 paper).
It looks just like the RSA protocol we already know but in reverse.

\begin{enumerate}
    \item (Key generation) Samantha chooses secret primes $p$ and $q$. She chooses verification exponent $e$ such that
    \[
        \gcd(e, (p-1)(q-1)) = 1.
    \]
    ...
    \]
    and keeps it secret.
    She publishes $N = pq$ and $e$.

    \item (Signing) Samantha signs document $D$ by computing
    \[
        S \equiv D^d \pmod N.
    \]

    \item (Verification) Victor accepts the signature as valid if and only if $S^e\equiv D\pmod N$.
\end{enumerate}
...
\]
by Euler's theorem.
Just like with the RSA messaging protocol, if Eve can factor $N$, then she can recover the signing exponent from the verification exponent.

\begin{remark}
    Technically, Eve doesn't have to factor $N$.
    She just needs to be able to compute the $e$-th root of the document to forge its signature.
\end{remark}

\begin{remark}
    We can again get a minor speedup by computing the inverse of $e$ modulo $(p-1)(q-1)/\gcd(p-1, q-1)$ instead.
\end{remark}
...
\subsection{ElGamal Signatures and DSA}
We can build a signature protocol based on the discrete logarithm problem too, but it's not quite as straightforward as with RSA (but still pretty simple).
Here's one based on ElGamal.

\begin{enumerate}
    \item (Parameter creation) Samantha (or a trusted party) publishes a large prime $p$ and a primitive root $g$ modulo $p$.

    \item (Key generation) Samantha chooses $1\leq a\leq p-1$ to be her \emph{signing key}.
    She publishes $A\equiv g^a\pmod p$, the verification key.

    \item (Signing) Samantha chooses a document $D\pmod{p-1}$.
    She then chooses a random $1<k<p$ such that $\gcd(k, p-1) = 1$.
    Finally, she computes the signature in two pieces $(S_1, S_2)$ as follows.
    \begin{align*}
        S_1 &\equiv g^k\pmod p\\
        S_2 &\equiv (D-aS_1)k^{-1}\pmod{p-1}.
    \end{align*}

    \item (Verification) Victor accepts the signature as valid if and only if
    \[
        A^{S_1}S_1^{S_2} \equiv g^D\pmod p.
    \]
\end{enumerate}

To see why this algorithm works, suppose Samantha signs the document $D$ and obtains $S_1$ and $S_2$ as above.
Then
\begin{align*}
    A^{S_1}S_1^{S_2} &\equiv g^{aS_1}g^{kS_2}\\
    & \equiv g^{aS_1 + k(D-aS_1)k^{-1}}\\
    &\equiv g^d\pmod p.
\end{align*}

Notice that since the quantity $S_2$ appears in the exponent, it makes sense to consider it modulo $p-1$.
It's also clear that if Eve can solve the discrete logarithm problem, then she can compute the signing key and totally break this system.
Does she really need to solve the DLP though?
What she really needs are integers $x$ and $y$ such that
\[
    A^xx^y \equiv g^D\pmod p.
\]
We can definitely solve this if we can solve the DLP: take the logarithm of both sides to the base $g$ to obtain
\[
    x\log_gA + y\log_gx \equiv D\pmod {p-1}.
\]
Then just plug in an arbitrary $x$ and solve this for $y$.

One drawback of this signature protocol is that the signature is pretty long.
In practice, $p$ is between 1000 and 2000 bits, so signatures are between 2000 and 4000 bits.
We now describe an algorithm that gives much shorter signatures.

The issue is that $p$ needs to be really large for the ElGamal algorithm to be secure against attacks like Pohlig-Hellman or index calculus (we didn't talk about this second one).
Instead of working in all of $\F_p^\times$, we pass to a subgroup of order $q$, where $q$ is a prime that's considerably smaller than $p$.
If it happens to be the case that solving the DLP in this subgroup isn't any easier than solving it in $\F_p^\times$, then we can obtain shorter signatures by using elements from this subgroup instead (since elements of a smaller group will take fewer bits to specify).
This is the idea behind the Digital Signature Algorithm (DSA).

% Samantha starts by choosing primes $p$ and $q$ with
% \[
%     p\equiv 1\pmod q.
% \]
% In practice, $p$ is between 1000 and 2000 bits while $q$ is between 160 and 320 bits.
% She then chooses an element $g\in \F_p^\times$ of order $q$.
% She can do this by first taking a primitive root $g_1$ of $\F_p^\times$ and setting
% \[
%     g = g_1^{(p-1)/q}.
% \]
% The rest proceeds like the ElGamal signature protocol with some minor twists.

% Samantha chooses a secret exponent $a$ and publishes
% \[
%     A \equiv g^a\pmod p
% \]
% along with $p$, $q$, and $g$.
% If she wants to sign the document $1\leq D < q$,
% she chooses a random $1<k<q$ and computes
% \[
%     S_1 \equiv (g^k \pmod p) \pmod q\qquad\text{and}\qquad S_2\equiv (D+aS_1)k^{-1}\pmod q.
% \]
% This looks just like an ElGamal signature, except $S_1$ and $S_2$ take only $\log q$ bits instead of $\log p$ bits to store.

\begin{enumerate}
    \item (Parameter creation) Samantha (or a trusted party) chooses large primes $p$ and $q$ satisfying $p\equiv 1\pmod q$ and an element $g$ of order $q$ modulo $p$.
    She can do this by first taking a primitive root $g_1$ of $\F_p^\times$ and setting
    \[
        g = g_1^{(p-1)/q}.
    \]

    \item (Key creation) Samantha chooses a secret signing key $1 \leq a < q$.
    She computes $A \equiv g^q\pmod p$ and publishes the verification key $A$.

    \item (Signing) Samantha chooses a document $D \pmod q$.
    She chooses a random $1< k < q$ and computes the signature
    \begin{align*}
        S_1 &\equiv (g^k\pmod p)\pmod q\\
        S_2 &\equiv (D+aS_1)k^{-1}\pmod q.
    \end{align*}

    \item (Verification) Victor computes
    \begin{align*}
...
\end{enumerate}
In practice, $p$ is between 1000 and 2000 bits while $q$ is between 160 and 320 bits.
If Samantha signs $D$ and $S_1, S_2$ and $V_1, V_2$ are as above, then
\begin{align*}
    g^{V_1}A^{V_2}\pmod p &\equiv g^{DS_2^{-1}}g^{aS_1S_2^{-1}}\\
    &\equiv g^{S_2^{-1}(D + aS_1)}\\
    &\equiv g^k\pmod p.
\end{align*}
The last line follows from the fact that $S_2 \equiv (D+aS_1)k^{-1}\pmod q$.
We then have that
\[
    \left(g^{V_1}A^{V_2}\pmod p\right)\pmod q \equiv \left(g^k \pmod p\right)\pmod q \equiv S_1.
\]
