...
\]
If $k$ is of moderate size (for a computer), say around 1000 bits (around 300 digits), then the time it would take to complete this algorithm would be greater than the estimated age of the universe, even if you reduced modulo $m$ after each step (if you didn't, then the integer $a^k$ would take up more bit than there are particles in the universe by some estimates).
