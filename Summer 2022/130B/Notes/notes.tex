...
\begin{example}
    Suppose $X_1$ and $X_2$ are independent $\Unif [0,1]$ random variables.
    Let $Y_1 = X_1 + X_2$ and $Y_2 = X_1 - X_2$.
    Let's find the joint density function of $Y_1$ and $Y_2$.

    In the context of the theorem, $g_1(x_1, x_2) = x_1+x_2$ and $g_2(x_1, x_2) = x_1 - x_2$.
    ...
    Consequently,
    \[
        f_{Y_1, Y_2}(y_1, y_2) = f_{X_1, X_2}(x_1, x_2)|J(x_1, x_2)|^{-1} = 1\cdot |-2|^{-1} = \frac{1}{2}.
    \]
...
\begin{example}
    ...
    \]
    when $X$ and $Y$ are both positive (we need to change $\Theta$ when we're in the other quadrants by adding the appropriate multiple of $\pi/2$).
